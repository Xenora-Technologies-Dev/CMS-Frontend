...
export function DoctorForm({ mode, doctorId }: DoctorFormProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setUserId(doctor.user.id);
      setEditForm({
        licenseNumber: doctor.licenseNumber ?? '',
        specialization: doctor.specialization ?? '',
        bio: doctor.bio ?? '',
        colorCode: doctor.colorCode ?? '#7C3AED',
        consultationStartTime: doctor.consultationStartTime ?? '09:00',
        consultationEndTime: doctor.consultationEndTime ?? '17:00',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === 'create') {
        ...
        const user: CreatedDoctorUser = result.user;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <PageActions
        backHref={mode === 'edit' && doctorId ? `/admin/doctors/${doctorId}` : '/admin/doctors'}
        backLabel="← Back"
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-2">
          <Button type="submit" disabled={saving}>
            {saving ? 'Saving…' : mode === 'create' ? 'Create doctor' : 'Save changes'}
          </Button>
